...
def plot_metrics(*, titles, number_of_plots, file_path_to_save, metrics, plot_type):
      ...
      if plot_type == 'all':
            ...
            if len(end_to_end_delay_fifo)>0:
                  ax[0].plot(ranged_values_fifo, end_to_end_delay_fifo, linewidth=2.5)
                  ax[0].set_xlabel('Steps', fontsize=12)
                  ax[0].set_ylabel('End to End Delay', fontsize=12)
                  ax[0].set_title(f'{end_to_end_title}', fontsize=12, fontweight='bold')
            ...
            if len(serving_ratio_fifo):
                  ax[1].plot(ranged_values_serving_fifo, serving_ratio_fifo)
                  ax[1].set_xlabel('Steps', fontsize = 12)
                  ax[1].set_ylabel('Serving Ratio', fontsize = 12)
                  ax[1].set_title(f'{serving_ratio_title}', fontsize = 12, fontweight='bold')
...
time_out_delay_file_path_fifo = os.path.join("run_fifo_scenario_2_lr_0.001", "time_out_delay", "time_out_delay0.pkl")
